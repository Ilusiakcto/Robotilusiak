...
def _align(arm, state, new_position, name, threshold, trigger=None):
    ...
    if trigger == "gripper":
        # Check if gripper is active (trigger pressed)
        gripper_position = new_position[-1]  # Last value is gripper's position
        gripper_val = gripper_position.as_py() if hasattr(gripper_position, 'as_py') else float(gripper_position)
        
        # V1 gripper: slide joint, 0.044m (open) to 0.0m (closed)
        # Consider gripping if gripper < 0.03m (trigger > ~30%)
        if gripper_val < 0.044:  # V1 slide joint (meters)
            is_gripping = gripper_val < 0.03  # Trigger pressed enough
        else:
            # V2 gripper: hinge joint (radians)
            if name == "right_arm":
                is_gripping = gripper_val > np.deg2rad(-5)
            elif name == "left_arm":
                is_gripping = gripper_val < np.deg2rad(5)
            else:
                is_gripping = False
